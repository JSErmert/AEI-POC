"""
Polished final slide deck for the Diabetes Risk Prediction project.
- Same 10-slide structure as outputs/final_slide_deck.pptx
- Replaces table/bullet slides with chart-driven layouts where possible
- Embeds the validated decision-tree image
- All numbers come from the validated R run (runtime_output_full.md)
- Brand palette: navy #0B2A4A, rust #C1441E, teal #1F7A8C, amber, neutral greys
"""
...
DECK = OUT / "final_slide_deck_polished.pptx"
...
def set_text(tf, text, *, size=18, bold=False, color=GREY_DARK,
    ...
    """Replace text frame content with a single paragraph styled uniformly."""
    tf.clear()
    p = tf.paragraphs[0]
    p.alignment = align
    run = p.add_run()
...
def add_rounded_card(slide, left, top, width, height, *,
    ...
    shp.adjustments[0] = 0.05  # mild rounding
    fill_solid(shp, fill)
    if border is None:
        no_line(shp)
    else:
...
def slide_2_why():
    s = add_blank()
    add_slide_title(s, "Why This Matters")
...
def slide_3_dataset():
    s = add_blank()
    add_slide_title(s, "Dataset and Response Variable")
    ...
    # Right: chart
    s.shapes.add_picture(str(OUT / "chart_class_imbalance.png"),
                         Inches(6.2), Inches(1.7),
                         width=Inches(6.7))

    # Callout under chart
    callout = add_rounded_card(s, Inches(6.2), Inches(4.5),
                               Inches(6.7), Inches(2.0),
                               fill=RUST_TINT, border=RUST, border_w=1.25)
    add_textbox(s, Inches(6.5), Inches(4.65), Inches(6.2), Inches(0.4),
                "KEY TAKEAWAY", size=11, bold=True, color=RUST)
    add_textbox(s, Inches(6.5), Inches(4.95), Inches(6.2), Inches(1.4),
                "Severe class imbalance — 84.24 % NoDiabetes, 1.83 % Prediabetes, 13.93 % Diabetes — is the single most important property of this dataset and shapes how every model should be read.",
...
def slide_4_variables():
    s = add_blank()
    add_slide_title(s, "Variables Used")
...
def slide_5_eda():
    s = add_blank()
    add_slide_title(s, "Preliminary Analysis  —  EDA highlights")

    # 6 finding rows in 2x3 grid
    findings = [
        ("BMI",         "Median rises 27  →  30  →  31"),
        ("Age",         "Median age category 8  →  9  →  10"),
        ("GenHlth",     "Markedly worse self-rated health in Diabetes group"),
        ("HighBP",      "~3× more common in Diabetes group than NoDiabetes"),
        ("HighChol",    "Same direction at slightly smaller magnitude"),
        ("PhysActivity","Lower in Diabetes group; Sex effect present but weaker"),
    ]
    ...
    for idx, (label, body) in enumerate(findings):
        r, c = divmod(idx, 3)
        x = grid_left + c * (cell_w + gap_x)
        y = grid_top + r * (cell_h + gap_y)
        card = add_rounded_card(s, x, y, cell_w, cell_h, fill=GREY_BG)
        ...
        add_textbox(s, x + Inches(0.3), y + Inches(0.85),
                    cell_w - Inches(0.6), cell_h - Inches(1.0),
                    body, size=13, color=GREY_DARK)

    # Strongest signals callout
...
def slide_6_methods():
    ...
    left = Inches(0.5)
    for m in methods:
        card = add_rounded_card(s, left, card_top, card_w, card_h, fill=WHITE,
...
def slide_7_comparison():
    s = add_blank()
    add_slide_title(s, "Model Comparison  —  Test accuracy")

    # Chart
    s.shapes.add_picture(str(OUT / "chart_model_comparison.png"),
                         Inches(0.5), Inches(1.7), width=Inches(8.6))

    # Right column callout
    callout = add_rounded_card(s, Inches(9.4), Inches(1.7),
    ...
    add_textbox(s, Inches(9.65), Inches(4.25), Inches(3.0), Inches(2.0),
...
def slide_8_tree():
    s = add_blank()
    add_slide_title(s, "Decision Tree  —  Structure")

    # Tree image: 1400x900 native (1.556 aspect ratio).
    # Constrain by HEIGHT so it fits between title and caption without clipping.
    img_h = Inches(4.7)
    img_w = Inches(4.7 * 1400 / 900)  # ≈ 7.31 in
    img_left = (SW - img_w) / 2
    img_top = Inches(1.55)
    ...
    # Caption sits below the image with safe margin from footer
    cap_top = Inches(6.40)
    cap = add_rounded_card(s, Inches(0.5), cap_top,
...
def slide_9_importance():
    s = add_blank()
    add_slide_title(s, "Variable Importance and Per-Class Story")

    # Left: variable importance chart
    s.shapes.add_picture(str(OUT / "chart_variable_importance.png"),
                         Inches(0.5), Inches(1.65),
                         width=Inches(7.0))

    # Right column - sensitivity chart + key insight
    s.shapes.add_picture(str(OUT / "chart_diabetes_sensitivity.png"),
                         Inches(7.7), Inches(1.65),
                         width=Inches(5.4))

    # Insight callout below sensitivity chart
    ...
    add_textbox(s, Inches(0.5), Inches(6.7), Inches(7.0), Inches(0.4),
                "Top 5 (rust) align directly with the EDA findings and with Type II diabetes risk-factor literature.",
                size=12, italic=True, color=GREY_MID)
...
def slide_10_conclusion():
    s = add_blank()
    add_slide_title(s, "Practical Value and Conclusion")

    # Two operational cards
    # Left: Rule-out branch
    card_l = add_rounded_card(s, Inches(0.5), Inches(1.7),
                              Inches(6.1), Inches(3.4),
                              fill=GREY_BG)
    add_textbox(s, Inches(0.85), Inches(1.95), Inches(5.5), Inches(0.4),
                "RULE-OUT BRANCH",
                size=11, bold=True, color=TEAL)
    add_textbox(s, Inches(0.85), Inches(2.25), Inches(5.5), Inches(1.0),
                "101,437", size=42, bold=True, color=NAVY)
    add_textbox(s, Inches(0.85), Inches(3.20), Inches(5.5), Inches(0.45),
                "patients with HighBP = 0",
                size=14, color=GREY_DARK)
    add_accent_line(s, Inches(0.85), Inches(3.75), Inches(0.7), color=TEAL)
    add_textbox(s, Inches(0.85), Inches(3.85), Inches(5.5), Inches(1.2),
                "Only 6 % Diabetes rate. Safe to deprioritize from intensive screening.",
                size=14, color=GREY_DARK)

    # Right: High-risk leaf
    card_r = add_rounded_card(s, Inches(6.8), Inches(1.7),
                              Inches(6.1), Inches(3.4),
    ...
    add_textbox(s, Inches(7.15), Inches(2.25), Inches(5.5), Inches(1.0),
                "3,961", size=42, bold=True, color=RUST)
    add_textbox(s, Inches(7.15), Inches(3.20), Inches(5.5), Inches(0.45),
                "BMI ≥ 35  +  HighChol  +  poor GenHlth  +  HighBP",
                size=12, color=GREY_DARK)
    add_accent_line(s, Inches(7.15), Inches(3.75), Inches(0.7), color=RUST)
    add_textbox(s, Inches(7.15), Inches(3.85), Inches(5.5), Inches(1.2),
